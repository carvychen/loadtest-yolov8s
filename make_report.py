...
bullet(doc, "按成本效率(QPS per $/hr)衡量,RTX PRO 6000 1/4 切片(NC24)以 275.4 QPS/$ 最优。")
bullet(doc, "RTX(NC24)成本效率约为 T4 的 1.55 倍、A10 的 3.16 倍。")
...
bullet(doc, "取每张卡在 p99 ≤ 50 ms 约束内吞吐最高的操作点作为其 QPS@SLA;成本效率 = QPS@SLA ÷ $/hr。")

# ── 三、评测结果 ──
doc.add_heading("三、评测结果", level=1)
make_table(doc,
    ["机型 (SKU)", "QPS @ SLA", "p99 (ms)", "并发", "$/hr", "QPS/$"],
    [["NC24lds_xl_RTXPRO6000BSE_v6", "311.2", "44.7", "12", "1.130", "275.4"],
     ["NC36lds_xl_RTXPRO6000BSE_v6", "311.2", "44.7", "12", "1.243", "250.4"],
     ["NC16as_T4_v3", "226.7", "27.5", "6", "1.276", "177.6"],
     ["NV36ads_A10_v5", "362.4", "39.8", "13", "4.160", "87.1"]],
    widths=[2.4, 0.95, 0.85, 0.6, 0.75, 0.8],
    highlight_row=0)
para(doc,
     "A10 单机绝对吞吐最高(362 QPS),但单价 $4.16/hr 偏贵,成本效率最低;T4 单机吞吐最低。"
     "RTX 1/4 切片以最低单价拿到次高吞吐,成本效率居首。",
     size=9.5, after=8)
...
para(doc,
     "QPS/$ 已折算机器数量:服务固定需求所需车队成本 = 需求 ÷ (QPS/$),该值越高车队越省,"
     "与单机吞吐高低无关。", size=9.5, after=4)
make_table(doc,
    ["机型 (SKU)", "QPS/$", "车队 $/hr", "相对 RTX(NC24)"],
    [["NC24lds_xl_RTXPRO6000BSE_v6", "275.4", "36.3", "1.00×(基准)"],
     ["NC36lds_xl_RTXPRO6000BSE_v6", "250.4", "39.9", "1.10×"],
     ["NC16as_T4_v3", "177.6", "56.3", "1.55×"],
     ["NV36ads_A10_v5", "87.1", "114.8", "3.16×"]],
    widths=[2.6, 0.95, 1.05, 1.4],
    highlight_row=0)
